...
def test_wave_2D():
    ...
    relative_error_file = Path(mor_results_filepath) / "test_relative_errors_wave.txt"
...
def test_wave_2D_new():
    ...
    nn_save_filepath = filepaths['checkpoints'] / "wave_2D_CNN_256x256_p_16_t_01_11_2025-14_02_55_scaled_circular_padding_warmrestarts.pt"
    network_parameters_file = filepaths['network_parameters'] / "wave_2D_CNN_256x256_p_16_t_01_11_2025-14_02_55_scaled_circular_padding_warmrestarts.pkl"
    ...
    for i in range(config.n_timesteps):
        ...
        # Quasi-Newton solve
        u_new = LSPG_quasi_newton(model, u_n1, mu_test, config.dt, experiment.fom, u_ref, scaled_data, tol=1e-6)
        u_approx.append(u_new)
    ...
    # Save results
    result_file = filepaths['mor_results'] / f'approx_full_deep_galerkin_p_{config.p_red}'
    ...
    with error_file.open("a") as f:
        f.write(f"Deep-Galerkin\t{config.p_red}\t{mu_test_val}\t{metrics['relative_error_total']}\n")
